...
class TestMainFailWell(unittest.TestCase):
    ...
    def testDataDiffNumCols(self):
        input_file = os.path.join(TEST_DATA_DIR, "OTU_data_imbalanced.xlsx")
        test_input = ["-d", input_file]
        if logger.isEnabledFor(logging.DEBUG):
            main(test_input)
        with capture_stderr(main, test_input) as output:
            self.assertTrue("Imbalanced input data" in output)
...
class TestDataProcessAnalysis(unittest.TestCase):
    def testSampleData(self):
        ...
        analysis_mean_vals, analysis_sd_vals = data_process_analysis(xlsx_data)
        expected_mean_vals = pd.read_excel(os.path.join(TEST_DATA_DIR, "OTU_data_results"),
                                           sheet_name='mean', index_col=0)
        expected_sd_vals = pd.read_excel(os.path.join(TEST_DATA_DIR, "OTU_data_results"),
    ...
    def testSampleData2(self):
        ...
        analysis_mean_vals, analysis_sd_vals = data_process_analysis(xlsx_data)
        expected_mean_vals = pd.read_excel(os.path.join(TEST_DATA_DIR, "OTU_data2_results"),
                                           sheet_name='mean', index_col=0)
        expected_sd_vals = pd.read_excel(os.path.join(TEST_DATA_DIR, "OTU_data2_results"),
                                         sheet_name='sd', index_col=0)
        self.assertTrue(np.allclose(analysis_mean_vals, expected_mean_vals) and
                        np.allclose(analysis_sd_vals, expected_sd_vals))
